...
import { offerBannerService, OfferBanner } from '../services/offerBanner.service';
import { Plus, Edit, Trash2, Image as ImageIcon, CheckCircle, Upload, Sparkles, BadgePercent } from 'lucide-react';

const API_BASE_URL = import.meta.env.VITE_API_URL || 'http://localhost:3000';

export default function OfferBannerManagement() {
  const [offerBanners, setOfferBanners] = useState<OfferBanner[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [showModal, setShowModal] = useState(false);
  const [editingOfferBanner, setEditingOfferBanner] = useState<OfferBanner | null>(null);
  ...
  const loadOfferBanners = async () => {
    try {
      setLoading(true);
      setError('');
      const data = await offerBannerService.getAllOfferBanners();
      setOfferBanners(data);
    } catch (err: any) {
      setError(err.response?.data?.error || err.message || 'Failed to load offer banners');
      setOfferBanners([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleEdit = (offerBanner: OfferBanner) => {
    setEditingOfferBanner(offerBanner);
    setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!editingOfferBanner && !formData.file) {
    ...
    try {
      setUploading(true);
      if (editingOfferBanner) {
        // Update existing offer banner
        await offerBannerService.updateOfferBanner(editingOfferBanner.id, {
          title: formData.title || undefined,
        });
      } else {
        // Upload new offer banner
        if (!formData.file) {
          alert('Please select an image file');
          return;
        }
        await offerBannerService.uploadOfferBanner(formData.file, formData.title || undefined);
      }
      setShowModal(false);
      loadOfferBanners();
    } catch (err: any) {
      alert(err.response?.data?.error || 'Failed to save offer banner');
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await offerBannerService.deleteOfferBanner(id);
      loadOfferBanners();
    } catch (err: any) {
      alert(err.response?.data?.error || 'Failed to delete offer banner');
    }
  };

  const handleSetActive = async (id: string) => {
    try {
      await offerBannerService.updateOfferBanner(id, { isActive: true });
      loadOfferBanners();
    } catch (err: any) {
      alert(err.response?.data?.error || 'Failed to set offer banner as active');
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-3xl sm:text-4xl font-bold text-gradient-brand">Offer Banner Management</h1>
            {offerBanners && (
              <p className="text-sm text-gray-600 dark:text-gray-400 mt-1 flex items-center space-x-2">
                <Sparkles className="w-4 h-4 text-brand-500" />
                <span>
                  <strong>{offerBanners.length}</strong>{' '}
                  {offerBanners.length === 1 ? 'offer banner' : 'offer banners'} total
                </span>
              </p>
            )}
          </div>
        </div>
        <button onClick={handleCreate} className="btn-brand flex items-center justify-center space-x-2">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center h-64 space-y-4">
          ...
          <p className="text-gray-600 dark:text-gray-400 font-medium animate-pulse">Loading offer banners...</p>
        </div>
      ) : (
          ...
          {offerBanners.length > 0 ? (
              ...
              >
                ...
                <div className="relative w-full h-48 bg-gradient-to-br from-gray-100 to-gray-200 dark:from-navy-800 dark:to-navy-900 overflow-hidden">
                  {offerBanner.imageUrl ? (
                    <img
                      src={getImageUrl(offerBanner.imageUrl)}
                ...
                <div className="p-4">
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">
                    {offerBanner.title || 'Untitled Offer Banner'}
                  </h3>
                  <p className="text-xs text-gray-600 dark:text-gray-400 mb-4">
                    Created: {new Date(offerBanner.createdAt).toLocaleDateString()}
                  </p>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {!offerBanner.isActive && (
                      <button
                        onClick={() => handleSetActive(offerBanner.id)}
                    ...
                    <button
                      onClick={() => handleEdit(offerBanner)}
                    ...
                    <button
                      onClick={() => handleDelete(offerBanner.id)}
            ...
            <div className="col-span-full card p-8 text-center border border-brand-400/20">
              <div className="flex flex-col items-center space-y-4">
                ...
                <p className="text-sm font-semibold text-gray-900 dark:text-white">No offer banners found</p>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto overflow-x-hidden modal-container">
          ...
          <div className="relative z-50 card border-2 border-brand-400/30 dark:border-cyan-500/30 w-full max-w-md sm:max-w-lg md:max-w-xl shadow-2xl shadow-brand-500/20 m-3 sm:m-4 md:m-6 mt-4 sm:mt-6 md:mt-8 mb-4 sm:mb-8 p-4 sm:p-5 md:p-6 lg:p-8 modal-content-zoom-safe">
            <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-gradient-brand mb-4 sm:mb-5 md:mb-6 text-zoom-safe truncate min-w-0">
              {editingOfferBanner ? 'Edit Offer Banner' : 'Upload Offer Banner'}
            </h3>
            <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4 md:space-y-5 w-full min-w-0 max-w-full">
              <div>
                ...
                <input
                  ...
                  placeholder="Enter offer banner title"
                    ...
                    >
                      {formData.file ? (
                        ...
                        <div className="text-center p-4">
                          ...
                          <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">PNG, JPG, WEBP up to 10MB</p>
                        </div>
                      )}
                    </label>
                  </div>
              ...
              {editingOfferBanner && (
                <div>
                  ...
                  <div className="w-full h-48 bg-gradient-to-br from-gray-100 to-gray-200 dark:from-navy-800 dark:to-navy-900 rounded-lg overflow-hidden">
                    <img
                      src={getImageUrl(editingOfferBanner.imageUrl)}
                      alt={editingOfferBanner.title || 'Offer Banner'}
                      className="w-full h-full object-cover"
                    />
                  </div>
                </div>
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 pt-2 sm:pt-3">
                ...
                >
                  {uploading ? 'Uploading...' : editingOfferBanner ? 'Update' : 'Upload'}
                </button>
                <button
                  type="button"
