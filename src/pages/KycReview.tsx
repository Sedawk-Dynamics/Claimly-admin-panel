    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
        <div className="flex items-center space-x-3 sm:space-x-4">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-gradient-brand">KYC Review</h1>
            <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 mt-1">Review and verify Aadhaar and PAN documents</p>
          </div>
        </div>
        <button
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center h-64 space-y-4">
          ...
          <p className="text-gray-600 dark:text-gray-400 font-medium animate-pulse text-sm sm:text-base">Loading KYC documents...</p>
        </div>
      ) : (
        <>
          {/* Desktop Table View */}
          ...
          <div className="lg:hidden space-y-4">
            {records?.data && records.data.length > 0 ? (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <div className="flex-1 min-w-0">
                        <div className="text-base sm:text-lg font-bold text-gray-900 dark:text-white truncate">{user.name}</div>
                        <div className="text-xs text-gray-500 dark:text-gray-400 mt-1 break-all">ID: {user.id}</div>
                      </div>
                    ...
                    <div className="space-y-2 mb-4">
                      ...
                      {user.documents.length > 0 && (
                        <div className="text-xs text-gray-500 dark:text-gray-400">
                          Uploaded: {new Date(user.documents[0].uploadedAt).toLocaleDateString()}
                        </div>
                      )}
                    </div>
              ...
              <div className="card p-8 text-center border border-brand-400/20">
                <div className="flex flex-col items-center justify-center">
                  <ShieldCheck className="w-12 h-12 text-gray-300 dark:text-navy-600 mb-3" />
                  <p className="text-sm text-gray-500 dark:text-gray-400">No KYC records found matching your criteria.</p>
                </div>
              </div>
            )}
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-3 card p-3 sm:p-4 border border-brand-400/20">
                ...
                <div className="flex space-x-2">
                  <button
                    onClick={() => setPage(page - 1)}
                    disabled={page === 1}
                    className="px-3 sm:px-4 py-2 border border-gray-300 dark:border-navy-600 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-white dark:hover:bg-navy-700 text-gray-700 dark:text-gray-300 transition-colors"
                    aria-label="Previous page"
                  >
                    <ChevronLeft className="w-4 h-4" />
                  </button>
          ...
          <div className="relative z-50 w-full max-w-md sm:max-w-lg md:max-w-2xl lg:max-w-4xl bg-white dark:bg-navy-900 rounded-2xl sm:rounded-3xl shadow-2xl flex flex-col max-h-[95vh] sm:max-h-[90vh] m-3 sm:m-4 md:m-6 mt-4 sm:mt-6 md:mt-8 mb-4 sm:mb-8">
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-4 p-4 sm:p-6 border-b border-gray-100 dark:border-navy-700">
              <div className="min-w-0 flex-1">
                ...
                <h2 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white truncate">{selectedUser.name}</h2>
            ...
            <div className="p-4 sm:p-6 overflow-y-auto space-y-4 sm:space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                <div className="p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-gray-200 dark:border-navy-700 bg-gray-50 dark:bg-navy-800/40">
                  ...
                  <p className="text-sm sm:text-lg font-bold text-gray-900 dark:text-white mt-1 break-all">{selectedUser.id}</p>
                </div>
                <div className="p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-gray-200 dark:border-navy-700 bg-gray-50 dark:bg-navy-800/40">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {selectedUser.pendingDocuments.length > 0 ? (
                      selectedUser.pendingDocuments.map((doc) => (
                        <span
                          key={doc}
                          className="px-2 sm:px-3 py-1 text-xs font-semibold rounded-full bg-yellow-100 text-yellow-800 dark:bg-yellow-500/20 dark:text-yellow-200"
                        >
                          {doc}
                        </span>
                      ))
                    ) : (
                      <span className="text-xs sm:text-sm text-green-600 dark:text-green-400">All required documents uploaded</span>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div>
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
                  <div>
                    ...
                    <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">Review each document before verifying</p>
                  </div>
                ...
                {selectedUser.documents.length > 0 ? (
                  ...
                  <div className="p-4 sm:p-6 rounded-xl sm:rounded-2xl border border-dashed border-gray-300 dark:border-navy-700 text-center space-y-3">
                    <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">No documents uploaded for this user.</p>
